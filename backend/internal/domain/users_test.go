...
// TestUserReviewsCascadeDelete checks the `constraint:OnDelete:CASCADE` tag
// on User.Reviews. This assumes BaseUUID does NOT include a DeletedAt
// field — none of the BaseUUID-embedding models shown so far (Session,
// OAuthAccount, Notification, Review, User, UserWatchProgress) define
// their own DeletedAt either, unlike Movie/Show/Season/Episode which add
// it explicitly. If that assumption is wrong and BaseUUID does have soft
// delete, db.Delete(&user) below will be a soft delete (an UPDATE), the
// same way Season's did, and this test will fail the same way the
// original cascade test did — that's a useful signal, not just test
// flakiness, so if it fails, let me know and I'll add an explicit
// DeleteUserCascade helper like DeleteSeasonCascade/DeleteShowCascade.
func TestUserReviewsCascadeDelete(t *testing.T) {
	...
	if err := db.AutoMigrate(&User{}, &Review{}); err != nil {
		t.Fatalf("failed to migrate User/Review: %v", err)
	}

	u := newTestUser("cascade-delete")
	if err := db.Create(&u).Error; err != nil {
		t.Fatalf("unexpected error creating user: %v", err)
	}
	...
	review := Review{
		UserID:         u.ID,
		Rating:         7,
		Title:          "Review to be cascade-deleted",
		ReviewableID:   1,
	...
	var count int64
	db.Model(&Review{}).Where("user_id = ?", u.ID).Count(&count)
	if count != 0 {
		t.Errorf("expected review to cascade-delete with its user, %d remain", count)
	}
}
